...
const Scoreboard = () => {
    ...
    const fetchScoreData = async () => {
        try {
            ...
            // Backend returns: [{ _id: "Spartans", totalPoints: 150 }, ...]
            const formattedLeaderboard = leaderboardRes.data.map(item => ({
                name: item._id,
                points: item.totalPoints
            }));

            // 2. Process Recent Scores for Tables
...
const handleViewWinners = () => {
    ...
    // Filter scores for positive and negative tables
    const positiveScores = recentScores.filter(s => s.position !== "Negative");
    const negativeScores = recentScores.filter(s => s.position === "Negative");

    return (
        <VerticalPatternBackground>
            {/* Main responsive container with cream background and Montserrat font */}
            <div className="relative z-10 font-['Montserrat'] text-stone-900 pb-20">
                ...
                <main className="max-w-7xl mx-auto px-4 pt-10 space-y-20">
                    ...
                    <section className="mx-auto w-full max-w-4xl relative">
                        ...
                        <div className="w-full bg-white border-4 border-stone-900 relative p-4 h-[600px] md:h-[700px]">
                            <h2 className="text-3xl font-extrabold text-stone-900 text-center mb-6 border-b-2 border-stone-200 pb-3 flex items-center justify-center gap-2">
                                <MdEmojiEvents className="text-desi-saffron" size={32}/> House Standings
                            </h2>
                            <div className="w-full h-[85%]">
                                {/* Chart Component using Live Data */}
