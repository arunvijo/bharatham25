...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[3000]">
      ...
      {/* MAIN NAVBAR CONTENT */}
      <nav
        className={`relative z-30 w-full px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between transition-colors duration-300 ${
          isNavOpen ? 'bg-primary' : 'bg-cream shadow-md'
        }`}
      >
        {/* BHARATHAM26 TEXT - Centered in navbar, same level as buttons */}
        <div 
          className="absolute top-0 left-0 w-full h-full flex items-center justify-center pointer-events-none z-10"
        >
          <h1 className={`font-mont font-semibold text-center leading-none transition-colors duration-300 text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl ${
        ...
        {/* CENTER: LOGO TEXT (Hidden when nav.svg is present) */}
        <h1 className={`absolute left-1/2 -translate-x-1/2 font-mont font-semibold text-center leading-none transition-colors duration-200 pointer-events-none text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl ${isNavOpen ? "text-yellow" : "text-black"}`}>
          {/* Empty - BHARATHAM26 text is now at the top of the nav */}
        </h1>
        ...
        <div className="flex items-center gap-2 sm:gap-4 z-50">
          ...
          >
            ...
            <div className="sm:hidden flex items-center justify-center w-8 h-8 rounded-full bg-black text-white hover:bg-desi-saffron transition-colors">
               {isAuthenticated ? <MdLogout size={16} /> : <MdLogin size={16} />}
            </div>

            {/* DESKTOP VIEW: Full SVG Button */}
            <div className="hidden sm:block relative w-[140px] md:w-[160px] aspect-[169/58]">
      ...
      {/* NAV.SVG LOGO CONTAINER - Below navbar */}
      <div 
        className={`w-full relative z-40 pointer-events-none transition-opacity duration-300 ${
          isNavOpen ? 'opacity-0' : 'opacity-100'
        }`}
      >
        {/* SVG Background - positioned naturally */}
        <div className="flex justify-center items-center">
          <img
            src="/images/nav.svg"
            alt="Logo Background"
            className="w-[200px] sm:w-[250px] md:w-[300px] lg:w-[350px] xl:w-[400px] h-auto -mt-2 drop-shadow-md"
          />
        </div>
      </div>
      ...
      <div className={`fixed inset-0 bg-primary flex items-center justify-center ${isClosing ? "opacity-0 transition-opacity duration-1000" : isNavOpen ? "opacity-100 transition-opacity duration-300" : "opacity-0"} ${isNavOpen || isClosing ? "pointer-events-auto" : "pointer-events-none"}`}>
        <div className="w-11/12 max-w-[1800px] flex flex-wrap justify-center gap-6 sm:gap-10 font-mont text-4xl sm:text-6xl md:text-display-nav lg:text-7xl xl:text-8xl text-white text-center">
